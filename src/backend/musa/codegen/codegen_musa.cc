...
constexpr const char *kMUSAGlobalBarrierState = "__tvm_global_barrier_state";
...
bool UseMusaNative16BitVector(DataType t) {
  return (t.is_float16() || t.is_bfloat16()) &&
...
CodeGenTileLangMUSA::CodeGenTileLangMUSA() {
  ...
  vid_global_barrier_state_ = name_supply_->FreshName(kMUSAGlobalBarrierState);
  vid_global_barrier_expect_ = name_supply_->FreshName("__barrier_expect");
  ICHECK_EQ(vid_global_barrier_state_, kMUSAGlobalBarrierState);
}
...
std::string CodeGenTileLangMUSA::Finish() {
  ...
  if (need_murand_kernel_h_) {
    decl_stream << "#include <murand_kernel.h>\n";
  }
...
void CodeGenTileLangMUSA::VisitStmt_(const tirx::ForNode *op) {
...
void CodeGenTileLangMUSA::VisitExpr_(const CastNode *op, std::ostream &os) {
  ...
  stream << ' ' << sret << ";\n";
  std::string src;
  auto GetSrc = [&]() -> const std::string & {
    if (src.empty()) {
      src = SSAGetID(PrintExpr(op->value), from_ty);
    }
    return src;
  };
  ...
  auto PrintVectorizedCast = [&](const std::string &cast_func,
                                 ...
                                 const std::string &dst_type, int chunk_lanes,
                                 const std::string &extra_args = "",
                                 bool src_needs_reinterpret = false,
                                 bool dst_needs_reinterpret = false) {
    ICHECK_GT(chunk_lanes, 0);
    ICHECK_EQ(lanes % chunk_lanes, 0);
    ...
    const std::string &src_value = GetSrc();
    int num_chunks = lanes / chunk_lanes;
    for (int i = 0; i < num_chunks; ++i) {
      PrintIndent();
      stream << "(" << dst_cast << "(&" << sret << "))[" << i
             << "] = " << cast_func << "((" << src_cast << "(&" << src_value
             << "))[" << i << "]" << extra_args << ");\n";
    }
    os << sret;
  };
  ...
  if (from_ty.is_float16() && target_ty.is_float() && target_ty.bits() == 32) {
    if (lanes == 2) {
      ...
      PrintVectorizedCast("tl::cvt_half_to_float_x4", "tl::half4_t", "float4",
                          4);
      return;
    }
  }
  ...
  if (from_ty.is_float() && from_ty.bits() == 32 && target_ty.is_float16()) {
    if (lanes == 2) {
      ...
      PrintVectorizedCast("tl::cvt_float_to_half_x4", "float4", "tl::half4_t",
                          4);
      return;
    }
  }
  ...
  if (from_ty.is_bfloat16() && target_ty.is_float() && target_ty.bits() == 32) {
    if (lanes == 2) {
      ...
      PrintVectorizedCast("tl::cvt_bfloat16_to_float_x4", "tl::bfloat164_t",
                          "float4", 4, "", true, false);
      return;
    }
  }
  ...
  if (from_ty.is_float16() &&
    ...
    if (lanes == 2) {
      ...
      PrintVectorizedCast(
          ...
          "tl::half4_t",
          IsMusaE4M3Type(target_ty) ? "fp8_e4_4_t" : "fp8_e5_4_t", 4);
      return;
    }
  }
  ...
  if ((IsMusaE4M3Type(from_ty) || IsMusaE5M2Type(from_ty)) &&
    ...
    if (lanes == 2) {
      ...
      PrintVectorizedCast(IsMusaE4M3Type(from_ty)
                              ? "tl::cvt_fp8e4m3_to_half_x4"
  ...
  for (int i = 0, lanes = from_ty.lanes(); i < lanes; ++i) {
    ...
    val << ")(";
    PrintVecElemLoad(GetSrc(), from_ty, i, val);
    val << ")";
...
void CodeGenTileLangMUSA::VisitExpr_(const CallNode *op, std::ostream &os) {
  ...
  if (op->op.same_as(tl::make_robust_desc())) {
    ICHECK_EQ(op->args.size(), 2);
    os << "tl::make_robust_desc(" << PrintExpr(op->args[0]) << ", "
       << PrintExpr(op->args[1]) << ")";
    return;
  }
  ...
  if (op->op.same_as(tl::mul_half_float_to_bfloat16_x4())) {
    ...
    std::string robust_base = this->PrintExpr(op->args[3]);
    std::string robust_size = this->PrintExpr(op->args[4]);
    this->PrintIndent();
    if (op->args.size() == 5U) {
      this->stream << "tl::cp_async_gs_robust<" << size << ">(" << dst << ", "
                   << src << ", " << robust_base << ", " << robust_size
                   << ");\n";
    } else {
    ...
    if (op->args.size() == 3U || op->args.size() == 4U) {
      ...
      ICHECK(op->args.size() == 5U || op->args.size() == 6U)
          << "ptx_cp_async expects 3/4 args (ptr form) or 5/6 args (offset "
             "form), but got "
          << op->args.size();
      std::string dst = this->PrintExpr(op->args[0]);
    ...
    if (barrier_count > 0) {
      next_mbarrier_id_ += barrier_count;
      ICHECK_LE(next_mbarrier_id_, 63)
          << "MUSA async barrier IDs must be in [1, 63], but requested "
          << next_mbarrier_id_ << " barriers";
      this->PrintIndent();
    ...
    // MUSA cp.async synchronization relies on wait_group; there is no
    // separate mbarrier-coupled cp.async arrive intrinsic to emit here.
    ...
    this->stream << "tl::warpgroup_wait<" << std::to_string(num_mma)
                 << ">();\n";
  } else if (op->op.same_as(tl::lma_wait())) {
    print_extern_call_stmt("tl::lma_wait");
    ...
    */
    ...
    ICHECK(op->args.size() == 4 || op->args.size() == 5)
        << "tl_gemm expects 4 or 5 arguments <op_instance, A_ptr, B_ptr, "
           "C_ptr[, thread_idx_base]>, but got "
        << op->args.size();
    ...
    this->need_murand_kernel_h_ = true;
    this->murand_random_generator_state =
        name_supply_->FreshName("__random_generator_state");
    auto generator = op->args[3].as<StringImmNode>()->value;
    if (generator == "curandStateMRG32k3a_t") {
      generator = "murandStateMRG32k3a_t";
    } else if (generator == "curandStatePhilox4_32_10_t") {
      generator = "murandStatePhilox4_32_10_t";
    } else if (generator == "curandStateXORWOW_t") {
      generator = "murandStateXORWOW_t";
    }
    this->murand_random_generator_state_type = generator;
    this->PrintIndent();
    this->stream << this->murand_random_generator_state_type << " "
                 << this->murand_random_generator_state << ";\n";
    this->PrintIndent();
    this->stream << "murand_init(" << PrintExpr(op->args[0]) << ", "
                 << PrintExpr(op->args[1]) << ", " << PrintExpr(op->args[2])
                 << ", &" << this->murand_random_generator_state << ");\n";
  } else if (op->op.same_as(tl::rng_rand())) {
    this->need_murand_kernel_h_ = true;
    os << "murand(&" << this->murand_random_generator_state << ")";
  } else if (op->op.same_as(tl::rng_rand_float())) {
    this->need_murand_kernel_h_ = true;
    os << "murand_" << op->args[0].as<StringImmNode>()->value;
    if (op->dtype.bits() == 64) {
      os << "_double";
    }
    os << "(&" << this->murand_random_generator_state << ")";
...
void CodeGenTileLangMUSA::VisitStmt_(const AllocBufferNode *op) {
  ...
  if (scope.find("wmma.") == 0) {
    ...
    bool is_fp4_scalar_local = alloc_dtype.is_float4() &&
  ...
  if (scope == "shared.dyn") {
    ...
    if (scope == "shared") {
      ...
      int base_id = next_mbarrier_id_ + 1;
      next_mbarrier_id_ += static_cast<int>(constant_size);
      ICHECK_LE(next_mbarrier_id_, 63)
          << "MUSA async barrier IDs must be in [1, 63], but requested "
          << next_mbarrier_id_ << " barriers";
      mbarrier_base_ids_[op->buffer->data.get()] = base_id;
      this->PrintIndent();
      this->stream << "__musa_async_bar_record(" << constant_size << ");\n";
      PrintIndent();
...
void CodeGenTileLangMUSA::VisitStmt_(const BindNode *op) {
  const auto *call = op->value.as<CallNode>();
  if (call && call->op.same_as(tvm::tl::make_robust_desc())) {
    return;
  }
  CodeGenC::VisitStmt_(op);
}
...
void CodeGenTileLangMUSA::VisitStmt_(const EvaluateNode *op) {
  ...
  if (call && call->op.same_as(tvm::tl::layout_marker())) {
    return;
  }
  if (call && call->op.as<OpNode>() &&
      call->op.as<OpNode>()->name == kMUSAGlobalBarrierKInitOp) {
    PrintIndent();
...
void CodeGenTileLangMUSA::VisitExpr_(const BufferLoadNode *op,
  ...
  if (op->buffer.scope() == "shared.barrier") {
    auto it = mbarrier_base_ids_.find(buffer_var.get());
    ICHECK(it != mbarrier_base_ids_.end())
        << "Cannot find MUSA async barrier base ID for " << buffer_var;
    int base_id = it->second;
    if (const auto *imm = index.as<IntImmNode>()) {
      int barrier_id = base_id + static_cast<int>(imm->value);
      ICHECK_GE(barrier_id, 1);
      ICHECK_LE(barrier_id, 63);
      os << barrier_id;
    } else {
      os << "(" << PrintExpr(index) << " + " << base_id << ")";
    }
    return;
  }
...
void CodeGenTileLangMUSA::VisitExpr_(const BroadcastNode *op,
  ...
  if ((op->dtype.is_int() || op->dtype.is_uint()) && op->dtype.bits() == 8) {
    ...
    if (lanes == 4) {
      ...
      if (op->dtype.is_uint()) {
        os << "(uint)((uchar)(" << v << ") * 0x01010101u)";
      } else {
        os << "(int)((uchar)(" << v << ") * 0x01010101u)";
      }
      return;
    } else if (lanes == 32) {
      std::string v = PrintExpr(op->value);
      if (op->dtype.is_uint()) {
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          os << "(((unsigned long long)(uint)((uchar)(" << v
             << ") * 0x01010101u) << 32) | (unsigned long long)(uint)((uchar)("
             << v << ") * 0x01010101u))";
        }
        os << ")";
      } else {
