/*!
 * \file tl/backend/musa/op/copy_analysis.cc
...
bool HasSourceRobustDesc(const CopyNode &op) {
  return op.annotations.count("src_robust_desc") != 0;
}
...
PreferredCopyInstruction GetPreferredInstruction(const CopyNode &op) {
  auto prefer = GetStringAnnotation(op, kPreferInstruction);
  if (!prefer) {
    return PreferredCopyInstruction::kAuto;
  }
  if (*prefer == "tma") {
    return PreferredCopyInstruction::kTMA;
  }
  if (*prefer == "cp_async") {
    return PreferredCopyInstruction::kCPAsync;
  }
  if (*prefer == "sync") {
    return PreferredCopyInstruction::kSync;
  }
...
bool IsSharedLayoutRepresentableForTMABulkStore(const LayoutMap &layout_map,
                                                const Buffer &shared_tensor,
                                                arith::Analyzer *analyzer);
...
struct CopyFacts {
  ...
  bool has_source_robust_desc = false;
...
bool IsMusaTarget(Target target) {
  return target.defined() && TargetIsMusa(target);
}
...
std::string MakeAsyncUnavailableReason(const CopyNode &op, Target target) {
  ...
  if (!target.defined()) {
    ...
    oss << "T.async_copy is only supported on cp.async-capable targets. Got "
           "target="
        << target;
...
CopyFacts AnalyzeCopyFacts(const CopyNode &op, const CopyAnalysisContext &ctx) {
  ...
  facts.layout_dependent_tma_available =
      facts.has_layout_map && !ctx.buffer_oob;
  ...
  if (facts.has_layout_map && !IsSharedLayoutRepresentableForTMABulkStore(
                                  layout_map, op.src, analyzer)) {
...
CopyInstSelection SelectCopyInstForLowering(const CopyNode &op,
  ...
  // MUSA robust and robust-async loads are emitted from the normal-copy path,
  // which preserves src_robust_desc (and force_async_copy) as TIR attributes.
  // Do not let a pipeline-managed annotation redirect them to CPAsync.
  if (facts.has_source_robust_desc) {
  ...
  if (facts.prefer_instruction == PreferredCopyInstruction::kTMA) {
    ...
    CopyInst inst =
        SelectTmaInst(facts, /*allow_load=*/true, /*allow_store=*/true,
                      /*check_last_dim=*/true);
    return inst == CopyInst::kInvalid
               ? Unsupported("T.copy prefer_instruction=\"tma\" could not be honored: " +
  ...
  if (facts.prefer_instruction == PreferredCopyInstruction::kCPAsync) {
    if (!IsAutoAsyncCopyEnabled(/*default_enabled=*/true)) {
      return Unsupported(
          "T.copy prefer_instruction=\"cp_async\" conflicts with pass "
          "config tl.enable_async_copy=false.");
    }
    return facts.can_cp_async
               ? Supported(CopyInst::kCPAsync)
               : Unsupported("T.copy prefer_instruction=\"cp_async\" could not be honored: " +
...
bool IsPipelineManagedCPAsyncCopy(const CopyNode &op, Target target) {
  ...
  ctx.target = target;
  CopyFacts facts = AnalyzeCopyFacts(op, ctx);
  if (!facts.musa_target || facts.explicit_tma || facts.explicit_cp_async ||
      facts.prefer_instruction != PreferredCopyInstruction::kAuto) {
    return false;
  }
  return facts.can_cp_async;
}
