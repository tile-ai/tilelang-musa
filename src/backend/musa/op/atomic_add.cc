/*!
 * \file tl/backend/musa/op/atomic_add.cc
 * \brief MUSA implementation for tl.atomic_add lowering.
 */
...
struct AtomicAdd {
  ...
  static Stmt Lower(const AtomicAddNode &op, const LowerArgs &T,
                    arith::Analyzer *analyzer) {
    if (UseTMA(op)) {
      DLOG(WARNING) << "MUSA TME atomic_add is not supported on the current "
                       "backend; falling back to SIMT atomic_add.";
    }
