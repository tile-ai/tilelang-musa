...
struct MaxOp {
  ...
  TL_DEVICE bfloat16_t operator()(bfloat16_t const &x, bfloat16_t const &y) {
    return bfloat16_t(__hmax(__mt_bfloat16(x), __mt_bfloat16(y)));
  }
...
struct MinOp {
  ...
  TL_DEVICE mutlass::bfloat16_t operator()(bfloat16_t const &x,
                                           bfloat16_t const &y) {
    return bfloat16_t(__hmin(__mt_bfloat16(x), __mt_bfloat16(y)));
  }
...
struct SyncThreadsBarrier {
  template <int stage = 0> static TL_DEVICE void sync() { __syncthreads(); }
};

template <int barrier_id> struct NamedBarrier {
  template <int stage = 0> static TL_DEVICE void sync() {
    auto phase = __musa_async_arrive(barrier_id + stage);
    __musa_async_wait(barrier_id + stage, phase);
  }
};
