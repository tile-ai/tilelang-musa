...
#if defined(TL_MUSA_ENABLE_FP16)
TL_DEVICE half htan(half x) { return __float2half_rn(tanf(__half2float(x))); }
#endif
...
#if defined(TL_MUSA_ENABLE_FP16)
TL_DEVICE half RoundTiesAwayFromZero(half x) {
  return __float2half_rn(roundf(__half2float(x)));
}
#endif

#if defined(TL_MUSA_ENABLE_BF16)
TL_DEVICE mt_bfloat16 RoundTiesAwayFromZero(mt_bfloat16 x) {
  return __float2bfloat16(roundf(__bfloat162float(x)));
}
#endif
