...
#else
  return tl_f4{max(a[0], b[0]), max(a[1], b[1]), max(a[2], b[2]),
               max(a[3], b[3])};
#endif
}

TL_DEVICE tl_f4 min4(tl_f4 a, tl_f4 b) {
  return tl_f4{min(a[0], b[0]), min(a[1], b[1]), min(a[2], b[2]),
               min(a[3], b[3])};
}
...
TL_DEVICE tl_h4 max4(tl_h4 a, tl_h4 b) {
  ...
  return make_tl_h4(max(tl_h_elem_to_half(a[0]), tl_h_elem_to_half(b[0])),
                    max(tl_h_elem_to_half(a[1]), tl_h_elem_to_half(b[1])),
                    max(tl_h_elem_to_half(a[2]), tl_h_elem_to_half(b[2])),
                    max(tl_h_elem_to_half(a[3]), tl_h_elem_to_half(b[3])));
}

TL_DEVICE tl_h4 min4(tl_h4 a, tl_h4 b) {
  return make_tl_h4(min(tl_h_elem_to_half(a[0]), tl_h_elem_to_half(b[0])),
                    min(tl_h_elem_to_half(a[1]), tl_h_elem_to_half(b[1])),
                    min(tl_h_elem_to_half(a[2]), tl_h_elem_to_half(b[2])),
                    min(tl_h_elem_to_half(a[3]), tl_h_elem_to_half(b[3])));
}

TL_DEVICE tl_h4 abs4(tl_h4 a) {
  return make_tl_h4(abs(tl_h_elem_to_half(a[0])),
                    abs(tl_h_elem_to_half(a[1])),
                    abs(tl_h_elem_to_half(a[2])),
                    abs(tl_h_elem_to_half(a[3])));
}
...
TL_DEVICE tl_bf4 max4(tl_bf4 a, tl_bf4 b) {
  ...
  return make_tl_bf4(bfloat16_t(max(float(tl_bf_elem_to_bfloat16(a[0])),
                                    float(tl_bf_elem_to_bfloat16(b[0])))),
                     bfloat16_t(max(float(tl_bf_elem_to_bfloat16(a[1])),
                                    float(tl_bf_elem_to_bfloat16(b[1])))),
                     bfloat16_t(max(float(tl_bf_elem_to_bfloat16(a[2])),
                                    float(tl_bf_elem_to_bfloat16(b[2])))),
                     bfloat16_t(max(float(tl_bf_elem_to_bfloat16(a[3])),
                                    float(tl_bf_elem_to_bfloat16(b[3])))));
}

TL_DEVICE tl_bf4 min4(tl_bf4 a, tl_bf4 b) {
  return make_tl_bf4(bfloat16_t(min(float(tl_bf_elem_to_bfloat16(a[0])),
                                    float(tl_bf_elem_to_bfloat16(b[0])))),
                     bfloat16_t(min(float(tl_bf_elem_to_bfloat16(a[1])),
                                    float(tl_bf_elem_to_bfloat16(b[1])))),
                     bfloat16_t(min(float(tl_bf_elem_to_bfloat16(a[2])),
                                    float(tl_bf_elem_to_bfloat16(b[2])))),
                     bfloat16_t(min(float(tl_bf_elem_to_bfloat16(a[3])),
                                    float(tl_bf_elem_to_bfloat16(b[3])))));
}

TL_DEVICE tl_bf4 abs4(tl_bf4 a) {
  return make_tl_bf4(
      bfloat16_t(fabsf(float(tl_bf_elem_to_bfloat16(a[0])))),
      bfloat16_t(fabsf(float(tl_bf_elem_to_bfloat16(a[1])))),
