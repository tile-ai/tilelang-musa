...
std::string MUSAGetFP8Type(DataType type) {
  ...
  if (type.is_scalar()) {
    ...
    TVM_FFI_THROW(InternalError) << "Only support scalar and vector types of "
                                    "width (2, 4, 8, 16) for FP8";
  }
  stream << "__mt_fp8";
  std::string suffix;
...
void CodeGenMUSA::PrintFunctionSignature(const ffi::String &function_name,
                                         const PrimFunc &func,
                                         std::ostream &os) {
  auto calling_conv = func->GetAttr<Integer>(
...
std::string CodeGenMUSA::Finish() {
  ...
  musa_declare_vector_type_extensions(decl_stream, enable_fp16_, enable_bf16_,
  ...
  if (t.is_float()) {
    ...
    case 1: {
      if (t.is_scalar()) {
        ...
        TVM_FFI_THROW(InternalError)
            << "Cannot convert type " << t << " to MUSA type!";
      }
    }
    case 4: {
      if (t.is_scalar()) {
...
void CodeGenMUSA::PrintVecBinaryOp(const std::string &op, DataType t,
...
void CodeGenMUSA::PrintVecElemLoad(const std::string &vec, DataType t, int i,
  ...
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
    ...
    os << "([](__mt_fp4_storage_t v) { __mt_fp4_e2m1 t; t.__x = v; return t; "
          "})(("
       << vec << ".__x >> " << i * 4 << ") & 0xF)";
  } else {
    os << vec << "." << access[i];
...
void CodeGenMUSA::PrintStorageScope(const std::string &scope,
                                    std::ostream &os) { // NOLINT(*)
...
std::string CodeGenMUSA::CastFromTo(std::string value, DataType from,
                                    DataType target) {
  if (from == target)
    return value;
...
void CodeGenMUSA::PrintCallExtern(Type ret_type, ffi::String global_symbol,
  ...
  if (global_symbol == "tl::RoundTiesAwayFromZero" ||
      global_symbol == "RoundTiesAwayFromZero") {
    need_math_h_ = true;
  }
...
void CodeGenMUSA::VisitExpr_(const CallNode *op, std::ostream &os) {
  ...
  if (op->op.same_as(tl::tma_load())) {
    ...
    const auto index_map_func = tvm::ffi::Function::GetGlobal(
        "tir.index_map.shared_16x16_to_ldmatrix_32x8_layout");
...
void CodeGenMUSA::VisitExpr_(const RampNode *op, std::ostream &os) {
  ...
  TVM_FFI_CHECK_LE(lanes, 4, ValueError)
      << "Ramp of more than 4 lanes is not allowed.";
...
void CodeGenMUSA::VisitExpr_(const BroadcastNode *op,
                             std::ostream &os) { // NOLINT(*)
  int lanes = op->dtype.lanes();
...
void CodeGenMUSA::VisitExpr_(const SelectNode *op, std::ostream &os) {
  ...
  ICHECK(op->false_value->dtype == op->dtype &&
...
inline void PrintConst(const FloatImmNode *op, std::ostream &os,
  ...
  default:
    TVM_FFI_THROW(InternalError)
        << "Bad bit-width for float: " << op->dtype << "\n";
  }
}

void CodeGenMUSA::VisitExpr_(const FloatImmNode *op,
                             std::ostream &os) { // NOLINT(*)
  PrintConst(op, os, this);
}

void CodeGenMUSA::PrintWmmaScope(const std::string &scope, DataType t,
  ...
  if ((t.is_int() || t.is_uint()) && t.bits() < 8 && t.lanes() == 1) {
    ...
    if (t.is_int()) {
      if (t.bits() == 4) {
        ...
        TVM_FFI_THROW(InternalError)
            << "Unhandled interger type for wmma fragment!";
      }
    } else if (t.is_uint()) {
      if (t.bits() == 4) {
...
int32_t CodeGenMUSA::GetWmmaFragmentSize(const std::string &scope,
...
void CodeGenMUSA::HandleVolatileLoads(const std::string &value,
...
void CodeGenMUSA::PrintVecElemLoadExpr(DataType t, int i,
                                       const std::string &value,
                                       std::ostream &os) {
  ICHECK_GT(t.lanes(), 1);
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
...
std::string MTCompile(const std::string &code, const Target &target) {
  ...
  std::filesystem::path base_dir =
      std::filesystem::temp_directory_path() / "tvm";
  std::error_code dir_err;
  std::filesystem::create_directories(base_dir, dir_err);
  ICHECK(!dir_err) << "MTCompile: cannot create cache directory "
                   << base_dir.string() << ": " << dir_err.message();

  std::ostringstream name;
  name << "musa_kernel_" << support::GetProcessId() << "_"
       << counter.fetch_add(1);
...

  {
    ...
    ICHECK(ofs.good()) << "MTCompile: cannot open temp source file "
                       << source_path;
    ofs << code;
    ofs.close();
    ICHECK(!ofs.fail()) << "MTCompile: failed to flush source file "
                        << source_path;
  }
  ...
  TVM_FFI_CHECK(arch.has_value(), ValueError)
      << "MUSA target does not have arch attribute";
  ...
  TVM_FFI_CHECK(ifs.good(), RuntimeError)
      << "MTCompile: cannot open output file " << output_path;
  compiled.assign(std::istreambuf_iterator<char>(ifs),
...
ffi::Module BuildMUSA(IRModule mod, Target target) {
  ...
  TVM_FFI_CHECK(fcreate.has_value(), RuntimeError)
      << "ffi.Module.create.musa is not registered. Build TVM with "
         "USE_MUSA=ON.";
  return (*fcreate)(ffi::Bytes(mubin.data(), mubin.size()),
                    ffi::String("mubin"), ExtractFuncInfo(mod),
                    std::move(source_map))
      .cast<ffi::Module>();
}
