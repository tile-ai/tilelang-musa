 ...
 * \file musa/transform/async_copy_injector.cc
 */
...
class MUSAAsyncCopyInjector : public StmtMutator {
public:
  explicit MUSAAsyncCopyInjector(bool async_without_async_commit_wait)
      : async_without_async_commit_wait_(async_without_async_commit_wait) {}

  bool InjectedMUSAAsyncCopy() const { return injected_ptx_async_copy_; }
  ...
  Optional<Stmt> TryInjectMUSA(const BufferLoadNode *load,
  ...
  Stmt VisitStmt_(const BufferStoreNode *store) final {
    ...
    if (load) {
      Optional<Stmt> injected =
          TryInjectMUSA(load, store, predicate.defined(),
...
InjectMUSAAsyncCopy(const Stmt &body, bool async_without_async_commit_wait) {
  MUSAAsyncCopyInjector injector(async_without_async_commit_wait);
  Stmt injected = injector(body);
  return {injector.Finalize(injected), injector.InjectedMUSAAsyncCopy()};
}
