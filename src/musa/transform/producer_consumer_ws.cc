...
static musa::CopyInstSelection ClassifyWarpSpecializedCopy(const CopyNode *copy,
                                                           Target target) {
  if (copy == nullptr) {
...
static bool CheckPipelineManagedCPAsyncCopy(const CopyNode *copy,
  ...
  return musa::IsPipelineManagedAsyncCopy(*copy, target);
}
...
static bool IsSyncGlobalToSharedCopyLikeStmt(const Stmt &stmt, Target target) {
  ...
  musa::CopyInstSelection result = ClassifyWarpSpecializedCopy(copy, target);
  return HasGlobalToSharedCopyShape(copy) && result.supported &&
         !musa::CopyInstIsTME(result.inst) &&
         !musa::CopyInstIsAsync(result.inst);
}
...
static TileStmtKind ClassifyCopy(const CopyNode *copy, Target target) {
  ...
  musa::CopyInstSelection result = ClassifyWarpSpecializedCopy(copy, target);
  if (musa::CopyInstIsTME(result.inst)) {
    return TileStmtKind::kTmaProducer;
  }
  if (musa::CopyInstIsAsync(result.inst)) {
...
private:
  ...
  Stmt BuildWSBlock(
    ...
    // MP31 TME completion still requires a software arrival from the producer
    // warp-group, including pure-TME producer groups.
    ...
    if (!has_simt_producer && !has_cp_async_producer &&
        num_producer_groups > 0) {
      for (int g = 0; g < num_producer_groups; ++g) {
        int fwd_base = g * num_stages;
        producer_stmts.push_back(MakeArriveBarrier(
            barrier_buf, IntImm(DataType::Int(32), fwd_base) + p_stage_expr));
      }
    }
    // MUSA has no cp.async-to-mbarrier binding instruction.  Complete the
    // async-copy group first, then let the producer warp-group arrive on the
    // forward TME barrier.  This preserves the develop MP31 synchronization
    // protocol while keeping the newer producer grouping/rewrite structure.
    if (has_simt_producer || has_cp_async_producer) {
      producer_stmts.push_back(Evaluate(
          Call(DataType::Handle(), builtin::ptx_wait_group(), {Integer(0)})));
      for (int g = 0; g < num_producer_groups; ++g) {
        int fwd_base = g * num_stages;
        PrimExpr fwd_id = IntImm(DataType::Int(32), fwd_base) + p_stage_expr;
    ...
    // Keep consumer warps at physical offset zero for native MP31 SQMMA thread
    // numbering.  Place the single TME producer warp after the consumers.
    ...
    Stmt dummy_ws = IfThenElse(LT(thread_iv_->var, consumer_extent),
                               dummy_consumer, dummy_producer);
    dummy_ws =
    ...
    if (!extracted_producer_init_.empty() ||
      ...
      Stmt ws_body = IfThenElse(LT(thread_iv_->var, consumer_extent),
                                scoped_consumer, scoped_producer);
      ws_body =
  ...
  private:
    ...
    Stmt UpdateWSBranchContainer(const Stmt &container,
      ...
      branch_ptr->then_case =
          AppendToStmt(branch_ptr->then_case, consumer_postlude);
...
static bool IsTmeCompatibleLayout(const Layout &layout, const Buffer &buffer) {
  if (StructuralEqual()(layout, MakeLinearLayout(buffer->shape))) {
    return true;
  }
  return musa::AnalyzeSwizzleLayout(buffer, layout).swizzle_granularity !=
         musa::SwizzleGranularity::kNone;
}
...
private:
  ...
  bool HasTmaCompatibleLayout(const Buffer &dst) const {
    ...
    return IsTmeCompatibleLayout(layout, buf);
  }
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("tl.musa.transform.ProducerConsumerWarpSpecialized",
