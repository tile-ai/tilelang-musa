...
private:
  ...
  void VisitExpr_(const BufferLoadNode *op) final { host_evaluable_ = false; }
...
private:
  ...
  explicit FastDivmodLowerer(std::unordered_set<const VarNode *> host_vars)
  ...
  DivisorInfo &GetOrCreateDivisorInfo(const PrimExpr &divisor) {
    ...
    if (const auto *immediate = divisor.as<IntImmNode>()) {
      ...
      info.multiplier = Var("musa_fast_div_multiplier_" + std::to_string(index),
                            DataType::UInt(32));
  ...
  PrimExpr VisitExpr_(const CallNode *op) final {
    ...
    return Call(op->dtype,
                is_fast_div ? fast_div_precomputed() : fast_mod_precomputed(),
  ...
  Stmt VisitStmt_(const SeqStmtNode *op) final {
    ...
    for (size_t index = 0; index < rewritten.size(); ++index) {
      ...
      bool matching_kinds = remainder != nullptr &&
      ...
      fused.push_back(rewritten[index]);
      fused.push_back(
          Bind(remainder_bind->var, fused_remainder, remainder_bind->span));
      ++index;
    }
  ...
  static PrimExpr MakeCeilLog2(const PrimExpr &divisor) {
    ...
    // The host wrapper has no corresponding MUSA C API; use the compiler
    // builtin.
    PrimExpr leading_zeros =
        Cast(u32, Call(DataType::Int(32), builtin::call_pure_extern(),
                       {StringImm("__builtin_clz"), divisor_minus_one}));
    PrimExpr ceil_log2 = make_const(u32, 32) - leading_zeros;
    return Select(divisor == make_const(divisor.dtype(), 1), make_const(u32, 0),
                  ceil_log2);
  }
  ...
  Stmt PrependHostSetup(Stmt body) const {
    ...
    for (const DivisorInfo &info : divisors_) {
      ...
      setup.push_back(
          Bind(Downcast<Var>(info.multiplier), MakeMultiplier(info.divisor)));
      setup.push_back(
          Bind(Downcast<Var>(info.shift_right), MakeShiftRight(info.divisor)));
    }
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("tl.musa.transform.LowerFastDivmod", LowerFastDivmod);
}
