...
bool IsManualTmaBarrier(const PrimExpr &barrier_id) {
  if (const auto *call = barrier_id.as<CallNode>()) {
    return call->op.same_as(manual_tma_barrier()) && call->args.size() == 1;
  }
  return false;
}

PrimExpr UnwrapManualTmaBarrier(const PrimExpr &barrier_id) {
  if (const auto *call = barrier_id.as<CallNode>()) {
...
private:
  PrimExpr VisitExpr_(const CallNode *call) final {
    if (call->op.same_as(tma_load()) || call->op.same_as(tma_load_im2col())) {
      ...
      if (IsManualTmaBarrier(barrier_id)) {
...
private:
  void VisitExpr_(const CallNode *call) final {
    if (call->op.same_as(tma_load()) || call->op.same_as(tma_load_im2col())) {
      int barrier_arg_index = GetTmaBarrierArgIndex(call);
      if (IsManualTmaBarrier(call->args[barrier_arg_index])) {
        StmtExprVisitor::VisitExpr_(call);
        return;
      }
      if (Is1DTmaLoad(call)) {
        // 1D TMA load uses size_bytes directly in args[3].
...
private:
  ...
  PrimExpr VisitExpr_(const CallNode *op) {
    if (op->op.same_as(tma_load()) || op->op.same_as(tma_load_im2col())) {
      ...
      PrimExpr barrier_id = op->args[barrier_arg_index];
      if (IsManualTmaBarrier(barrier_id)) {
        return IRMutatorWithAnalyzer::VisitExpr_(op);
      }
      visited_tma_load_ = true;
      Array<PrimExpr> new_args = op->args;
      new_args.Set(barrier_arg_index, Call(DataType::Handle(), get_mbarrier(),
...
public:
  TmaBarrierCollector(Map<Var, Buffer> buffer_data_to_buffer,
      ...
      : buffer_data_to_buffer_(std::move(buffer_data_to_buffer)),
        manual_barrier_ids_(std::move(manual_barrier_ids)) {}
...
private:
  ...
  IntImm GetThreadExtentOrDefaultOne() {
    int64_t extent = 1;
    if (thread_var_.defined() &&
        analyzer_.const_int_bound.IsBound(thread_var_->var)) {
      auto const_int_bound = analyzer_.const_int_bound(thread_var_);
      extent = const_int_bound->max_value - const_int_bound->min_value + 1;
    }
    extent = std::max<int64_t>(extent, 1);
    return IntImm(DataType::Int(32), static_cast<int>(extent));
  }
  ...
  void VisitStmt_(const EvaluateNode *op) final {
    if (const auto *call = op->value.as<CallNode>()) {
      if (call->op.same_as(tma_load()) || call->op.same_as(tma_load_im2col())) {
        ...
        if (IsRecordedManualBarrier(barrier_id)) {
          StmtExprVisitor::VisitStmt_(op);
          return;
        }
        ...
        UpdateBarrierRange(barrier_id, GetThreadExtentOrDefaultOne());
...
public:
  ...
  const std::unordered_map<int, bool> &barrier_accumulate_flags() const {
    return barrier_accumulate_flags_;
  }
...
private:
  ...
  int GetCurrentThreadCount() {
    ...
    if (!thread_var_.defined()) {
      return std::max(fallback_thread_count_, 1);
    }
  ...
  void UpdateBarrierThreadCount(const PrimExpr &barrier_expr, int thread_count,
                                bool accumulate_old_count) {
    ...
    if (const auto *imm = normalized_barrier_expr.as<IntImmNode>()) {
      ...
      if (it == barrier_thread_counts_.end()) {
        ...
        barrier_accumulate_flags_[id] = accumulate_old_count;
      } else {
        it->second = std::max(it->second, thread_count);
        barrier_accumulate_flags_[id] =
            barrier_accumulate_flags_[id] || accumulate_old_count;
      }
      return;
    }
    ...
    if (const auto *call = op->condition.as<CallNode>()) {
      ...
      if (is_elect_if && !call->args.empty()) {
        if (const auto *imm = call->args[0].as<IntImmNode>()) {
          fallback_thread_count_ =
              std::max(fallback_thread_count_, static_cast<int>(imm->value));
        }
      }
    }
    if (is_elect_if) {
...
public:
  BarrierCreationRewriter(
      ...
      std::unordered_map<int, bool> barrier_accumulate_flags,
      int ensure_min_count = 0, PrimExpr default_barrier_thread_count = 1)
      : barrier_thread_counts_(std::move(barrier_thread_counts)),
        barrier_accumulate_flags_(std::move(barrier_accumulate_flags)),
    ...
    if (op->op.same_as(create_list_of_mbarrier())) {
      ...
      for (size_t i{0}; i < cur_n; ++i) {
        ...
        if (it != barrier_thread_counts_.end()) {
          ...
          auto mode_it = barrier_accumulate_flags_.find(static_cast<int>(i));
          bool accumulate_old_count =
              mode_it != barrier_accumulate_flags_.end() && mode_it->second;
          if (accumulate_old_count) {
            if (const auto *old_imm = op->args[i].as<IntImmNode>()) {
              thread_count += static_cast<int>(old_imm->value);
            }
          }
...
public:
  ...
  static PrimFunc Rewrite(PrimFunc f, arith::Analyzer *analyzer) {
    ...
    if (!single_warp_arrive) {
      ...
      barrier_thread_counts =
          arrive_thread_count_collector.barrier_thread_counts();
      barrier_accumulate_flags =
          arrive_thread_count_collector.barrier_accumulate_flags();
    }
    ...
    auto barrier_creation_rewriter = BarrierCreationRewriter(
        std::move(barrier_thread_counts), std::move(barrier_accumulate_flags),
