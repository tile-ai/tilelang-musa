...
def tl_fused_chunk_fwd_kernel(
    ...
    def fused_chunk_linear_attn_fwd(
        ...
        with T.Kernel(NV, NK, B * H) as (i_v, i_k, i_bh):
            ...
            k_A = T.alloc_shared([chunk_size, BK], dtype)
            k_B = T.alloc_shared([chunk_size, BK], dtype)
            ...
            for i in T.Pipelined(0, NT):
                ...
                T.copy(K[i_b, i * chunk_size : (i + 1) * chunk_size, i_h, i_k * BK : (i_k + 1) * BK], k_A)
                T.copy(K[i_b, i * chunk_size : (i + 1) * chunk_size, i_h, i_k * BK : (i_k + 1) * BK], k_B)
                T.copy(V[i_b, i * chunk_size : (i + 1) * chunk_size, i_h, i_v * BV : (i_v + 1) * BV], v)

                T.gemm(q, k_B, s, clear_accum=True, transpose_B=True)
                for row, col in T.Parallel(chunk_size, chunk_size):
                    s_shared[row, col] = T.if_then_else(row >= col, s[row, col], 0)

                T.gemm(s_shared, v, o, clear_accum=True)
                T.copy(h, h_shared)
                T.gemm(k_A, v, h, transpose_A=True)
                T.gemm(q, h_shared, o)
                T.copy(o, o_shared)
...
def tl_fused_chunk_fwd(q, k, v):
    ...
    o = torch.zeros((B, S, H, D), device="musa", dtype=torch.float32)
    h = kernel(q, k, v, o)
    return o, h
...
def main(B=1, S=512, H=16, D=128):
    q = torch.randn((B, S, H, D), device="musa", dtype=torch.float16)
    k = torch.randn((B, S, H, D), device="musa", dtype=torch.float16)
    v = torch.randn((B, S, H, D), device="musa", dtype=torch.float16)
