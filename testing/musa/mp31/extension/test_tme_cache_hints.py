...
def _assert_musa_tme_cache_hints(source, inner_hint, outer_hint):
...
def test_musa_tme_cache_hint_codegen(kwargs, expected):
    ...
    _, source = _compile_source(**kwargs)
    _assert_musa_tme_cache_hints(source, *expected)


@tilelang.testing.requires_musa_compute_version_ge(3, 1)
def test_musa_tme_cache_none_end_to_end():
    compiled, source = _compile_source(
        inner_cache_policy="cache_none",
        outer_cache_policy="cache_normal",
    )
    _assert_musa_tme_cache_hints(source, "CACHE_NONE", "CACHE_NORMAL")
