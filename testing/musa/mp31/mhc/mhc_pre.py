...
def mhc_pre(
    ...
    gemm_kernel = mhc_pre_gemm_sqrsum_tilelang(hc_mult3, hc_mult * hidden_size)
    gemm_kernel(
    ...
    fuse_kernel = mhc_pre_big_fuse_tilelang(
        hidden_size,
        rms_eps,
        hc_pre_eps,
        hc_sinkhorn_eps,
...
def benchmark(n: int, hidden_size: int, hc_mult: int) -> None:
    ...
    io_bytes = (
        ...
        + test_data["fn"].numel() * 4
        + test_data["hc_scale"].numel() * 4
        + test_data["hc_base"].numel() * 4
        + n * hc_mult * 4
        + n * hc_mult * hc_mult * 4
        + n * hidden_size * 2
    )
    ...
    print(f"[PERF] case=mhc_pre device={device} params=n={n},hidden_size={hidden_size},hc_mult={hc_mult}")
