    ...
    if trans_A and trans_B:
        ...
        def gemm(
            ...
            with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
                A_local = T.alloc_fragment((block_K, block_M), dtype)
                ...
                for k in T.serial(T.ceildiv(K, block_K)):
                    ...
                    T.wmma_gemm(A_local, B_local, C_local, transpose_A=True, transpose_B=True)
        ...
        def gemm(
            ...
            with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
                ...
                for k in T.serial(T.ceildiv(K, block_K)):
                    ...
                    T.wmma_gemm(A_local, B_local, C_local, transpose_A=True)
                T.copy(C_local, C[by * block_M, bx * block_N])
        ...
        def gemm(
            ...
            with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
                ...
                for k in T.serial(T.ceildiv(K, block_K)):
                    ...
                    T.copy(B[bx * block_N, k * block_K], B_local)
                    T.wmma_gemm(A_local, B_local, C_local, transpose_B=True)
                T.copy(C_local, C[by * block_M, bx * block_N])
    ...
    def gemm(
        ...
        with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
            ...
            for k in T.serial(T.ceildiv(K, block_K)):
                T.copy(A[by * block_M, k * block_K], A_local)
                T.copy(B[k * block_K, bx * block_N], B_local)
                T.wmma_gemm(A_local, B_local, C_local)
            T.copy(C_local, C[by * block_M, bx * block_N])
