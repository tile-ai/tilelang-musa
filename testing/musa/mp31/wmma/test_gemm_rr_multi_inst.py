...
def matmul(
    ...
    a_fragment_shape = (block_K, block_M) if trans_A else (block_M, block_K)
    b_fragment_shape = (block_N, block_K) if trans_B else (block_K, block_N)
    ...
    def gemm(
        ...
        with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
            A_fragment = T.alloc_fragment(a_fragment_shape, dtype)
            B_fragment = T.alloc_fragment(b_fragment_shape, dtype)
            ...
            for k in T.Pipelined(T.ceildiv(K, block_K), num_stages=stages):
                ...
                T.gemm(A_fragment, B_fragment, C_local, transpose_A=trans_A, transpose_B=trans_B)
...
type_block_threads_stages_list = [
    (torch.float16, 32, 32, 32, 32, 0),
    (torch.float16, 32, 32, 32, 128, 0),
    (torch.float16, 64, 64, 32, 128, 0),
]
test_params = [
    (
        elem_type,
