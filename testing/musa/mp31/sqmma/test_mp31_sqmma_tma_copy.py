    ...
    if trans_A and trans_B:
        ...
        def gemm(
            ...
            with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
                ...
                A_mbar = T.alloc_barrier(threads)
                B_mbar = T.alloc_barrier(threads)
                T.clear(C_local)
                for k in T.serial(T.ceildiv(K, block_K)):
                    T.tma_copy(A[k * block_K, by * block_M], A_shared, barrier=A_mbar)
                    T.tma_copy(B[bx * block_N, k * block_K], B_shared, barrier=B_mbar)
                    T.barrier_arrive(A_mbar)
                    T.barrier_arrive(B_mbar)
                    T.barrier_wait(A_mbar, k % 2)
                    T.barrier_wait(B_mbar, k % 2)
    ...
    if trans_A:
        ...
        def gemm(
            ...
            with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
                ...
                for k in T.serial(T.ceildiv(K, block_K)):
                    T.tma_copy(A[k * block_K, by * block_M], A_shared, barrier=A_mbar)
                    T.tma_copy(B[k * block_K, bx * block_N], B_shared, barrier=B_mbar)
                    T.barrier_arrive(A_mbar)
                    T.barrier_arrive(B_mbar)
                    T.barrier_wait(A_mbar, k % 2)
                    T.barrier_wait(B_mbar, k % 2)
    ...
    if trans_B:
        ...
        def gemm(
            ...
            with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
                ...
                T.clear(C_local)
                for k in T.serial(T.ceildiv(K, block_K)):
                    T.tma_copy(A[by * block_M, k * block_K], A_shared, barrier=A_mbar)
                    T.tma_copy(B[bx * block_N, k * block_K], B_shared, barrier=B_mbar)
                    T.barrier_arrive(A_mbar)
                    T.barrier_arrive(B_mbar)
                    T.barrier_wait(A_mbar, k % 2)
