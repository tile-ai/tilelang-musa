...
def test_mp31_tme_load_codegen():
    ...
    with target:
        artifact = tilelang.lower(_tme_load_1d, target=target, enable_device_compile=False)
...
@T.prim_func
def _tme_load_nonzero_outer_slice(
    A: T.Tensor((2, 10, 8, 16), "float32"),
    C: T.Tensor((8, 16), "float32"),
):
    with T.Kernel(1, threads=128):
        smem = T.alloc_shared((2, 10, 8, 16), "float32")
        barrier = T.alloc_barrier(128)
        T.tma_copy(A[1, 0, 0:8, 0:16], smem[1, 0, 0:8, 0:16], barrier=barrier)
        T.barrier_arrive(barrier)
        T.barrier_wait(barrier, 0)
...
def test_mp31_tme_load_ranks_codegen():
    ...
    for shape in shapes:
        with target:
            artifact = tilelang.lower(_make_tme_load(shape), target=target, enable_device_compile=False)
        source = artifact.kernel_source
        assert "tl::tme_load(" in source
        assert "tl_templates/musa/mp31/tme.h" in source
...
@tilelang.testing.requires_musa_compute_version_eq(3, 1)
def test_mp31_tme_load_nonzero_outer_slice():
    kernel = tilelang.compile(
        _tme_load_nonzero_outer_slice,
        out_idx=[1],
        target={"kind": "musa", "arch": "mp_31"},
        execution_backend="tvm_ffi",
    )
    values = torch.randn((2, 10, 8, 16), dtype=torch.float32)
    result = kernel(values.to("musa"))
    torch.musa.synchronize()
