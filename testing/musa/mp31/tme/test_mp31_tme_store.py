    ...
    def kernel(A: T.Tensor(shape, "float32"), C: T.Tensor(shape, "float32")):
        with T.Kernel(1, threads=128):
            ...
            if len(shape) == 1:
                ...
                for i0, i1, i2, i3 in T.Parallel(shape[0], shape[1], shape[2], shape[3]):
                    smem[i0, i1, i2, i3] = A[i0, i1, i2, i3]
            else:
                for i0, i1, i2, i3, i4 in T.Parallel(shape[0], shape[1], shape[2], shape[3], shape[4]):
...
@T.prim_func
def _tme_store_nonzero_outer_slice(
    A: T.Tensor((8, 16), "float32"),
    C: T.Tensor((2, 10, 8, 16), "float32"),
):
    with T.Kernel(1, threads=128):
        smem = T.alloc_shared((2, 10, 8, 16), "float32")
        for i, j in T.Parallel(8, 16):
            smem[1, 0, i, j] = A[i, j]
        T.sync_threads()
        T.tma_copy(smem[1, 0, 0:8, 0:16], C[1, 0, 0:8, 0:16])
        T.tma_store_wait(0)


@T.prim_func
def _tme_store_singleton_aligned_regions(
    A: T.Tensor((8, 16), "float32"),
    C: T.Tensor((2, 8, 3, 16), "float32"),
):
    with T.Kernel(1, threads=128):
        ...
        for i, j in T.Parallel(8, 16):
            smem[i, j] = A[i, j]
        T.sync_threads()
        T.tma_copy(smem, C[1, 0:8, 2:3, 0:16])
        T.tma_store_wait(0)
...
def test_mp31_tme_store_ranks_codegen():
    ...
    for shape in shapes:
        with target:
            artifact = tilelang.lower(_make_tme_store(shape), target=target, enable_device_compile=False)
...
@tilelang.testing.requires_musa_compute_version_eq(3, 1)
def test_mp31_tme_store_nonzero_outer_slice():
    kernel = tilelang.compile(
        _tme_store_nonzero_outer_slice,
        out_idx=[1],
        target={"kind": "musa", "arch": "mp_31"},
        execution_backend="tvm_ffi",
    )
    values = torch.randn((8, 16), dtype=torch.float32)
    result = kernel(values.to("musa"))
    torch.musa.synchronize()
    torch.testing.assert_close(result[1, 0].cpu(), values)


@tilelang.testing.requires_musa_compute_version_eq(3, 1)
def test_mp31_tme_store_singleton_aligned_regions():
    kernel = tilelang.compile(
        _tme_store_singleton_aligned_regions,
        out_idx=[1],
        target={"kind": "musa", "arch": "mp_31"},
        execution_backend="tvm_ffi",
    )
    ...
    result = kernel(values.to("musa"))
    torch.musa.synchronize()
    torch.testing.assert_close(result[1, :, 2, :].cpu(), values)
