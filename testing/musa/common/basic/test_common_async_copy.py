...
@T.prim_func
def _async_copy_conditional_kernel(
    src: T.Tensor((100,), T.float32),
    dst: T.Tensor((128,), T.float32),
):
    """The larger destination forces a source-boundary predicate."""
    with T.Kernel(1, threads=128):
        smem = T.alloc_shared((128,), T.float32)
        T.async_copy(src[0:100], smem)
        T.ptx_wait_group(0)
        T.sync_threads()
        T.copy(smem, dst)
...
def test_musa_async_copy_codegen(dtype):
    ...
    assert "#include <tl_templates/musa/common/async_copy.h>" in source
...
@tilelang.testing.requires_musa
def test_musa_async_copy_conditional_codegen_and_runtime():
    target = tvm.target.Target({"kind": "musa"})
    with target:
        artifact = tilelang.lower(
            _async_copy_conditional_kernel,
            target=target,
            enable_device_compile=False,
        )

    source = artifact.kernel_source
    assert "tl::cp_async_gs_conditional<4>" in source
    ...
    src = torch.arange(100, dtype=torch.float32)
