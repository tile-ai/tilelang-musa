...
def flashattn(
    batch, heads, k_heads, max_seqlen_kv, total_seqlen_k, dim, has_sink, block_N=128, block_H=32, num_split=1, num_stages=1, threads=128
):
    ...
    def flashattn_gqa_decode_no_split(
        ...
        with T.Kernel(batch, heads // valid_block_H, num_split, threads=threads) as (bid, hid, bz):
            ...
            for k in T.Pipelined(loop_range, num_stages=num_stages):
                ...
                temp_shared = T.alloc_shared([block_H, block_N], accum_dtype)
...
def test_varlen_decode_main(args):
    ...
    # test ordering.
    musa_devices = list(range(torch.musa.device_count()))
    with torch.random.fork_rng(devices=musa_devices):
        torch.manual_seed(0)
        if musa_devices:
        ...
        cu_seqlens_k = torch.zeros(batch_size + 1, device="musa", dtype=torch.int32)
        cu_seqlens_k[1:] = torch.cumsum(k_seqlens, dim=0).to(torch.int32).musa()
        total_k_tokens = cu_seqlens_k[-1].item()

        # Generate input tensors
        q = torch.randn(batch_size, q_heads, head_size, device="musa", dtype=dtype)
        k_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device="musa", dtype=dtype)
        v_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device="musa", dtype=dtype)
        sink = torch.randn(q_heads, device="musa", dtype=torch.float32) * 0.1 if args.test_sink else None
    ...
    k_padded = torch.zeros(batch_size, kv_heads, actual_max, head_size, device="musa", dtype=dtype)
    v_padded = torch.zeros(batch_size, kv_heads, actual_max, head_size, device="musa", dtype=dtype)
    for i in range(batch_size):
    ...
    # Compute reference
    O_ref, attn_weights = ref_attention(q, k_padded, v_padded, k_seqlens.musa(), q_heads, sink)
...
def speed_benchmark_decode_comparison(args):
    ...
    # Generate tensors
    q_decode = torch.randn(batch_size, q_heads, head_size, device="musa", dtype=dtype)
    k_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device="musa", dtype=dtype)
    v_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device="musa", dtype=dtype)
    sink = torch.randn(q_heads, device="musa", dtype=torch.float32) * 0.1 if args.test_sink else None
    if args.test_varlen:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--head_size", type=int, default=64, choices=[64, 128, 256], help="Head dimension")
