    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(max_seqlen_q, block_M), heads, batch_size, threads=threads) as (bx, by, bz):
            ...
            for k in T.Pipelined(loop_range, num_stages=num_stages):
                ...
                temp_shared = T.alloc_shared([block_M, block_N], accum_dtype)  # 使用与 acc_s 相同的类型（float32）
...
def main(
    ...
    kernel = flashattn(batch, groups, UQ, UKV, heads, dim, is_causal, block_M=64, block_N=64, num_stages=2, threads=256)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--q_seqlen", type=int, default=1024, help="query sequence length")
    parser.add_argument("--k_seqlen", type=int, default=1024, help="key/value sequence length")
