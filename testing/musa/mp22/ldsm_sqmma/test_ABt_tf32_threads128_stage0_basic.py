...
def get_tilelang_type(elem_type):
    type_map = {torch.float: "float32", torch.float16: "float16", torch.bfloat16: "bfloat16", torch.float8_e4m3fn: "float8_e4m3"}
    return type_map.get(elem_type)


elem_type_list = [torch.float]
size_list = [(4096, 4096, 4096)]
block_threads_stages_list = [
    # (16, 64, 8, 128, 0), qy2 required m and n musa be multiple of 32
    # (16, 64, 16, 128, 0), qy2 required m and n musa be multiple of 32
    # (16, 64, 32, 128, 0), qy2 required m and n musa be multiple of 32
    (32, 32, 8, 128, 0),
    (32, 32, 16, 128, 0),
    (32, 32, 32, 128, 0),
    (32, 64, 8, 128, 0),
    (32, 64, 16, 128, 0),
    (32, 64, 32, 128, 0),
    # (64, 16, 8, 128, 0), qy2 required m and n musa be multiple of 32
    # (64, 16, 16, 128, 0), qy2 required m and n musa be multiple of 32
    # (64, 16, 32, 128, 0), qy2 required m and n musa be multiple of 32
    (64, 32, 8, 128, 0),
    (64, 32, 16, 128, 0),
    (64, 32, 32, 128, 0),
    (64, 64, 8, 128, 0),
...
def test_mm_kernel(elem_type, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, threads, stages):
    device = "musa"
    A = torch.randn((M, K), dtype=torch.float, device=device).to(elem_type)
    B = torch.randn((N, K), dtype=torch.float, device=device).to(elem_type)
    ...
    ref_out = A @ B.T
    C = kernel(A, B)
    torch.testing.assert_close(ref_out.to(torch.float), C.to(torch.float), rtol=1.25e-1, atol=1.25e-1)
