...
def test_issue_1810_l2_persistent_float16_host_stub_no_half(monkeypatch):
    ...
    `annotate_l2_hit_ratio` lowers to MUSA stream access policy window calls.
    ...
    @T.prim_func
    def minimal_kernel(A: T.Tensor((1,), "float16")):
        with T.Kernel():
            T.annotate_l2_hit_ratio({A: 0.9})
            T.evaluate(0)

    kernel = tilelang.compile(minimal_kernel, execution_backend="tvm_ffi", target="musa")
    source = kernel.get_host_source()
    assert "__tvm_musa_stream_set_access_policy_window_packed" in source
    assert "__tvm_musa_stream_reset_access_policy_window_packed" in source
