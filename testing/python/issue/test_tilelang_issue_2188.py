...
def test_named_barrier_arrive_musa_guard():
    @T.prim_func
    def kernel(inp: T.Tensor((1,), T.int32), out: T.Tensor((1,), T.int32)):
        with T.Kernel(1, threads=64):
            T.named_barrier_arrive(10, 64)
            T.copy(inp, out)

    with pytest.raises(Exception, match="T.named_barrier_arrive is not supported on MUSA"):
        tilelang.compile(kernel, out_idx=[1], target="musa")
