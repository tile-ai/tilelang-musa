...
def test_subtype_fp4_dynamic_stride_store(block_size):
    ...
    slots = torch.randperm(num_blocks * block_size, dtype=torch.int32, device=device)[:n]

    # static (reference): strides known at compile time
    back_s, fp4_s = make_buf()
    s0 = fp4_s.stride(0) * 2  # byte stride to fp4-element stride
    s1 = fp4_s.stride(1) * 2
...
def test_subtype_fp4_scalar_store_codegen(n):
    """Scatter fp4 elements via indirection to force scalar stores."""
    ...
    perm = torch.randperm(n, dtype=torch.int32, device=device)

    scatter_kern(src, dst, perm)

    # Invert the permutation and check src[inv[j]] == dst[j] at byte level
    inv = torch.empty_like(perm)
    inv[perm] = torch.arange(n, dtype=torch.int32, device=device)
