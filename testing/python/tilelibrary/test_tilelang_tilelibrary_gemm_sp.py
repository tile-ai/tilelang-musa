...
def generate_dense_input(M, N, K, trans_A, trans_B, in_dtype):
    ...
    if is_int:
        ...
        A = randint_semi_sparse(M, K, low=low, high=high, dtype=T.dtype(in_dtype).as_torch(), device="musa", transposed=trans_A)
        B = torch.randint(size=(N, K) if trans_B else (K, N), low=low, high=high, dtype=T.dtype(in_dtype).as_torch(), device="musa")
    else:
        A = randn_semi_sparse(M, K, dtype=torch.float32, device="musa", transposed=trans_A).to(T.dtype(in_dtype).as_torch())
        B = torch.randn((N, K) if trans_B else (K, N), device="musa", dtype=torch.float32).to(T.dtype(in_dtype).as_torch())
    return A, B
