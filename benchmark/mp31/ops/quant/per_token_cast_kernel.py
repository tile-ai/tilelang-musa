            ...
            if in_config.with_sf:
                ...
                for i, j in T.Parallel(block_m, block_k // num_vectorize):
                    stage2_amax_fragment[i, j] = (
                        T.float32(stage1_amax_fragment[i, j])
                        * x_sf_fragment[i // in_config.sf_block[0], j * num_vectorize // in_config.sf_block[1]]
                    )

                # Reduce stage 2, using float for reduction
                stage2_amax_fragment_reshaped = T.reshape(
                    stage2_amax_fragment, [block_m, num_groups, block_k // num_vectorize // num_groups]
                )
                ...
                if not sf_only:
                    for i, j in T.Parallel(block_m, block_k):
                        ...
                        factor = (
                            x_sf_fragment[i // in_config.sf_block[0], j // in_config.sf_block[1]]
                ...
                if not sf_only:
                    for i, j in T.Parallel(block_m, block_k):
                        if direct_store:
                            out[pid_token * block_m + i, pid_hidden * block_k + j] = (
...
def per_token_cast_with_sf_only(
    ...
    return per_token_cast_impl(
        x, fmt, num_per_channels, x_block_size, use_tma_aligned_col_major_sf, round_sf, use_packed_ue8m0, sf_only=True
    )
