...
def _mhc_pass_configs(mode: str = "burst"):
    mode = mode.lower()
    if mode == "none":
        return None
    ...
    if mode not in ("safe", "burst", "aggressive"):
        raise ValueError(f"MHC TileLang pass config must be one of 'safe', 'burst', 'aggressive', or 'none', got {mode!r}")
    return pass_configs


def _tilelang_jit(name: str, pass_configs=None):
    try:
...
def _mhc_pre_big_fuse(
    ...
    @_tilelang_jit(
        ...
        pass_configs=_mhc_pass_configs(pass_config),
    )
    def mhc_pre_big_fuse(
        ...
        with T.Kernel(num_tokens, threads=threads) as pid:
            ...
            if T.get_thread_binding() < 32:
                ##################################################################
                # _mhc_pre_split_mixes_fwd (pre, post & comb)
                cm = T.alloc_fragment((mhc_mult, mhc_mult), T.float32)
                for j in T.Parallel(mhc_mult):
                    pre_mix_shared[j] = T.sigmoid(mixes_shared[j] * mhc_scale[0] + mhc_base[j]) + mhc_pre_eps
                for j in T.Parallel(mhc_mult):
