...
def format_record_label(record: dict[str, Any]) -> str:
    ...
    return f"{record['kernel']} {op_text} [{format_params(record['params'])}]"
...
def count_bytes(*tensors: torch.Tensor) -> int:
    total = 0
...
def benchmark_timer(fn, warmup: int = 100, rep: int = 100) -> float:
...
def import_ops():
    return import_module("benchmark.mp31.tilekernels")
...
def sequential_topk_mapping(
    ...
    token_topk_to_pos = torch.arange(
        num_expanded_tokens,
        dtype=torch.int32,
        device=device,
    ).reshape(num_tokens, num_topk)
    pos_to_expert = (torch.arange(num_expanded_tokens, dtype=torch.int32, device=device) % num_experts).contiguous()
...
def print_perf(record: dict[str, Any]) -> None:
    ...
    bandwidth_text = f"{record['bandwidth_gbs']:.4f} GB/s"
    print(f"{style('[PERF]', TermStyle.bold, TermStyle.blue)} {style(record['kernel'], TermStyle.bold)} {style(op_text, TermStyle.dim)}")
    print(f"  {style('time', TermStyle.cyan):<14} {style(time_text, TermStyle.bold)}")
    print(f"  {style('bandwidth', TermStyle.cyan):<14} {style(bandwidth_text, TermStyle.green)}")
    if "bytes_rw" in extras:
        bytes_detail = f"({extras['bytes_rw']})"
        print(f"  {style('bytes_rw', TermStyle.cyan):<14} {format_bytes(extras['bytes_rw'])} {style(bytes_detail, TermStyle.dim)}")
...
def print_sample_stats(sample_records: list[dict[str, Any]], median_time_us: float) -> None:
    ...
    mean_time_us = sum(times) / len(times)
    print(
        f"  {style('samples', TermStyle.cyan):<14} "
        f"n={len(times)} "
        f"median={median_time_us:.2f} us "
        f"mean={mean_time_us:.2f} us "
        f"min={min(times):.2f} us "
        f"max={max(times):.2f} us"
    )
