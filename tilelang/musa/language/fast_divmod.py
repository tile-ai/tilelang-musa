...
def _normalize_operands(dividend: PrimExpr, divisor: PrimExpr) -> tuple[PrimExpr, PrimExpr]:
    ...
    if dividend.dtype != divisor.dtype:
        raise TypeError(
            f"fast integer division requires dividend and divisor to have the same dtype, got {dividend.dtype} and {divisor.dtype}"
        )
    if DataType(dividend.dtype) != DataType("int32"):
        raise TypeError(f"fast integer division currently supports scalar int32 operands only, got {dividend.dtype}")
...
def fast_div(dividend: PrimExpr, divisor: PrimExpr) -> PrimExpr:
    ...
    return tirx.call_intrin(dividend.dtype, tirx.op.Op.get("tl.musa.fast_div"), dividend, divisor)
...
def fast_mod(dividend: PrimExpr, divisor: PrimExpr) -> PrimExpr:
    ...
    dividend, divisor = _normalize_operands(dividend, divisor)
    return tirx.call_intrin(dividend.dtype, tirx.op.Op.get("tl.musa.fast_mod"), dividend, divisor)
...
def fast_divmod(dividend: PrimExpr, divisor: PrimExpr) -> tuple[PrimExpr, PrimExpr]:
    ...
    dividend, divisor = _normalize_operands(dividend, divisor)
    return (
        tirx.call_intrin(dividend.dtype, tirx.op.Op.get("tl.musa.fast_div"), dividend, divisor),
        tirx.call_intrin(dividend.dtype, tirx.op.Op.get("tl.musa.fast_mod"), dividend, divisor),
    )
